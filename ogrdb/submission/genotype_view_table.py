    ...
    def td_contents(self, item, attr_list):
        ...
        if item.genotype_description.submission.species == 'Homo sapiens':
            ...
            for k, v in igpdb_genes.items():
                if item.nt_sequence.lower() in v or v in item.nt_sequence.lower():
                    bt_igpdb = '<button type="button" class="btn btn-xs text-ogrdb-info icon_back" data-bs-toggle="tooltip" title="Sequence matches IGPDB gene %s"><i class="bi bi-info-circle-fill"></i>&nbsp;</button>' % k
                    break
            ...
            if vdjbase_species in vdjbase_ref and locus in vdjbase_ref[vdjbase_species]:
                ...
                for vdjbase_name, (vdjbase_seq, vdjbase_count) in vdjbase_genes.items():
                    if item.nt_sequence.lower() in vdjbase_seq or vdjbase_seq in item.nt_sequence.lower():
                        bt_vdjbase = '<button type="button" name="vdjbasebtn" id="vdjbasebtn" class="btn btn-xs text-ogrdb-info icon_back"  onclick="window.open(%s)" data-bs-toggle="tooltip" title="Sequence matches VDJbase gene %s (found in %s subjects). Click to view in VDJbase."><i class="bi bi-info-circle-fill"></i>&nbsp;</button>' % \
        ...
        if item.sequence_id not in imgt_ref[item.genotype_description.submission.species]:
            if item.closest_reference not in imgt_ref[item.genotype_description.submission.species]:
                bt_ref_found = '<button type="button" class="btn btn-xs text-ogrdb-info icon_back" data-bs-toggle="tooltip" title="Nearest reference not found in IMGT reference set"><i class="bi bi-info-circle-fill"></i>&nbsp;</button>'
            else:
                for k,v in imgt_ref[item.genotype_description.submission.species].items():
                    if item.nt_sequence.lower() == v:
                        bt_imgt = '<button type="button" class="btn btn-xs text-ogrdb-info icon_back" data-bs-toggle="tooltip" title="Sequence matches IMGT gene %s"><i class="bi bi-info-circle-fill"></i>&nbsp;</button>' % k
                        break
                ...
                if mismatch > 20:
                    aligned = False
                    bt_indels = '<button type="button" class="btn btn-xs text-ogrdb-info icon_back" data-bs-toggle="tooltip" title="Sequence has indels/low match when compared to reference sequence"><i class="bi bi-info-circle-fill"></i>&nbsp;</button>'
                ...
                if aligned:
                    ...
                    if item.genotype_description.sequence_type == 'V' and item.genotype_description.locus+'V' in ref_codon_usage[item.genotype_description.submission.species]:
                        try:
                            ...
                            if len(q_codons) > 0:
                                bt_codon_usage = '<button type="button" class="btn btn-xs text-ogrdb-info icon_back" data-bs-toggle="tooltip" title="Amino Acid(s) previously unreported in this family: %s"><i class="bi bi-info-circle-fill"></i>&nbsp;</button>' % ", ".join(q_codons)

                        except:
                            bt_codon_usage = '<button type="button" class="btn btn-xs text-ogrdb-info icon_back" data-bs-toggle="tooltip" title="Error translating sequence: %s"><i class="bi bi-info-circle-fill"></i>&nbsp;</button>' % sys.exc_info()[0]
                    ...
                    if len(q_runs) > 0:
                        bt_runs = '<button type="button" class="btn btn-xs text-ogrdb-info icon_back" data-bs-toggle="tooltip" title="Possible repeated read errors at IMGT position(s) %s"><i class="bi bi-info-circle-fill"></i>&nbsp;</button>' % ", ".join(q_runs)
                    ...
                    if len(q_hotspots) > 0:
                        bt_hotspots = '<button type="button" class="btn btn-xs text-ogrdb-info icon_back" data-bs-toggle="tooltip" title="G/C SNP in RGYW/WRCY hotspot at IMGT position(s) %s"><i class="bi bi-info-circle-fill"></i>&nbsp;</button>' % ", ".join(q_hotspots)
