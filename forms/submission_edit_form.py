...
class SeqNameCol(StyledCol):
    def td_contents(self, item, attr_list):
        ...
        if item.sequence_details:
            ret += '<button type="button" class="btn btn-xs text-info icon_back" data-toggle="modal" data-target="#seqModal" data-sequence="%s" data-name="%s" data-fa="%s"><span class="glyphicon glyphicon-search"></span>&nbsp;</button>&nbsp;' \
